(function () {
  ...
  const coverImageBtn = document.querySelector('.add_logo_img_btn')

  const bookCoverRemoveBtn = document.querySelector(".remove_logo_img_btn")
  
  
  function renderCoverAndShowRemoveBtn(attachment) {
    let imgEl = `<img src="${attachment.sizes.medium.url}" />`
    const bookCoverThumb = document.querySelector('.logo_img_check')
  ...
  function bindSelectCover() {
    media.on('insert', function () { // frame이 post라면 insert, select 또는 입력하지 않았다면 select
      ...
      const bookCoverInput = document.querySelector("[name='meta[comp_logo_image]']")
      bookCoverInput.value = attachment.id
    });
  }

  function renderCoverAlreadyHas(){
    if(document.querySelector("[name='meta[comp_logo_image]']").value){
      wp.media.attachment(document.querySelector("[name='meta[comp_logo_image]']").value)
      .fetch()
  ...
  function bindRemoveCover() {
    bookCoverRemoveBtn.addEventListener('click', function() {
      document.querySelector("[name='meta[comp_logo_image]']").value = ''
